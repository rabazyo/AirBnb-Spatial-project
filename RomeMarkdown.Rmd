...
rome <- read.csv("~/GitHub/AirBnb-Spatial-project/data sets/rome_weekdays.csv")
str(rome)
```
...
summary_df <- data.frame(
  Variable = names(rome)[-c(1:7)],  
  Minimum = sapply(rome[, -c(1:7)], min),  
  Q1 = sapply(rome[, -c(1:7)], quantile, probs = 0.25),  
  Median = sapply(rome[, -c(1:7)], median),  
  Mean = sapply(rome[, -c(1:7)], mean),  
  Q3 = sapply(rome[, -c(1:7)], quantile, probs = 0.75),
  Maximum = sapply(rome[, -c(1:7)], max) 
)
...
summary_table <- gt(summary_df) %>%
  ...
  tab_header(
    ...
    subtitle = "Table displaying summary statistics for Rome dataset"
  )
...
# Histograms
hist(rome$realSum, main = "Distribution of Total Price", xlab = "Total Price (realSum)", col = "pink")
hist(rome$cleanliness_rating, main = "Distribution of Cleanliness Rating", xlab = "Cleanliness Rating", col = "lightgreen")
hist(rome$guest_satisfaction_overall, main = "Distribution of Guest Satisfaction", xlab = "Overall Guest Satisfaction", col = "salmon")
hist(rome$dist, main = "Distribution of Distance from City Center", xlab = "Distance (km)", col = "lightyellow")
hist(rome$metro_dist, main = "Distribution of Distance from Metro", xlab = "Distance (km)", col = "lavender")
```


```{r}
# Box plots
boxplot(rome$realSum, main = "Box Plot of Total Price", col = "pink")
# Box plot for cleanliness rating
boxplot(rome$cleanliness_rating, main = "Box Plot of Cleanliness Rating", col = "lightgreen")
# Box plot for guest satisfaction overall
boxplot(rome$guest_satisfaction_overall, main = "Box Plot of Guest Satisfaction Overall", col = "salmon")
# Box plot for distance from city center
boxplot(rome$dist, main = "Box Plot of Distance from City Center", col = "lightyellow")
# Box plot for distance from metro station
boxplot(rome$metro_dist, main = "Box Plot of Distance from Metro Station", col = "lavender")
```

```{r}
#Creating geomtetry points

bar_sf <- st_as_sf(rome, coords = c("lng", "lat"), crs = 4326)
st_geometry(bar_sf)
st_crs(bar_sf)
```
