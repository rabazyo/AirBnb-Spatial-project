...
form_pre <- bar_sf$guest_satisfaction_overall ~ log(bar_sf$realSum) + bar_sf$host_is_superhost + bar_sf$person_capacity  + bar_sf$room_type   + bar_sf$rest_index + bar_sf$attr_index + bar_sf$multi + bar_sf$biz + log(bar_sf$metro_dist)+ log(bar_sf$dist)
...
```{r}
form_price <- log(bar_sf$realSum) ~ bar_sf$guest_satisfaction_overall + bar_sf$host_is_superhost + bar_sf$person_capacity  + bar_sf$room_type   + bar_sf$rest_index + bar_sf$attr_index + bar_sf$multi + bar_sf$biz + log(bar_sf$metro_dist)+ log(bar_sf$dist)


GNM_pre <- sacsarlm(form_price, data=bar_sf, listw=lw, Durbin=update(form_price, ~ . - Metrop), control=list(pre_eig1=e, pre_eig2=e))
SAC_pre <- sacsarlm(form_price, data=bar_sf, listw=lw, control=list(pre_eig1=e, pre_eig2=e))
SDEM_pre <- errorsarlm(form_price, data=bar_sf, listw=lw, Durbin=update(form_price, ~ . - Metrop), control=list(pre_eig=e))
SDM_pre <- lagsarlm(form_price, data=bar_sf, listw=lw, Durbin=update(form_price, ~ . - Metrop), control=list(pre_eig=e))
SLM_pre <- lagsarlm(form_price, data=bar_sf, listw=lw, control=list(pre_eig=e))
SLX_pre <- lmSLX(form_price, data=bar_sf, listw=lw, Durbin=update(form_price, ~ . - Metrop))
OLS_pre <- lm(form_price, data=bar_sf)
```

```{r}
# Get coefficients, p-values, and R-squared for each model
coefficients_OLS <- coef(summary(OLS_pre))
p_values_OLS <- coefficients_OLS[, "Pr(>|t|)"]
r_squared_OLS <- summary(OLS_pre)$r.squared

coefficients_SLX <- coef(summary(SLX_pre))
p_values_SLX <- coefficients_SLX[, "Pr(>|t|)"]
r_squared_SLX <- summary(SLX_pre)$r.squared

coefficients_SDM <- coef(summary(SDM_pre))
p_values_SDM <- coefficients_SDM[, "Pr(>|z|)"]
r_squared_SDM <- summary(SDM_pre)$r.squared

coefficients_SAR <- coef(summary(sar1))
p_values_SAR <- coefficients_SAR[, "Pr(>|z|)"]
r_squared_SAR <- summary(sar1)$r.squared

# Combine coefficients, p-values, and R-squared into a single data frame
summary_combined <- data.frame(
  Model = c("OLS", "SLX", "SDM", "SAR"),
  R_squared = c(r_squared_OLS, r_squared_SLX, r_squared_SDM, r_squared_SAR),
  Coefficients = c(coefficients_OLS[, "Estimate"], coefficients_SLX[, "Estimate"], coefficients_SDM[, "Estimate"], coefficients_SAR[, "Estimate"]),
  P_value = c(p_values_OLS, p_values_SLX, p_values_SDM, p_values_SAR)
)
