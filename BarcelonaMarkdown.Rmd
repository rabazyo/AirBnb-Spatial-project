...
form_pre <- ubar_sf$guest_satisfaction_overall ~ log(ubar_sf$realSum)  + ubar_sf$host_is_superhost +ubar_sf$attr_index  + ubar_sf$person_capacity  + ubar_sf$dist +ubar_sf$room_type + ubar_sf$attr_index
...
```
```{r}
form_pre <- log(ubar_sf$realSum) ~ ubar_sf$guest_satisfaction_overall  + ubar_sf$host_is_superhost +ubar_sf$attr_index  + ubar_sf$person_capacity  + ubar_sf$dist + ubar_sf$metro_dist
```
...
##3 GMM estimation  
```{r}
SAC_gmm <- spreg(form_pre, data=ubar_sf, listw=lw, model="sarar")
