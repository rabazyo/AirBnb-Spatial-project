...
```{r}
#Combining Weekdays and Weekends Datasets for Athens into a single dataframe
athens_weekdays <- read.csv("~/GitHub/AirBnb-Spatial-project/data sets/athens_weekdays.csv")
athens_weekends <- read.csv("~/GitHub/AirBnb-Spatial-project/data sets/athens_weekends.csv")
...
summary_df <- data.frame(
  Variable = names(athens)[-c(1:7)],  
  Minimum = sapply(athens[, -c(1:7)], min),  
  Q1 = sapply(athens[, -c(1:7)], quantile, probs = 0.25),  
  Median = sapply(athens[, -c(1:7)], median),  
  Mean = sapply(athens[, -c(1:7)], mean),  
  Q3 = sapply(athens[, -c(1:7)], quantile, probs = 0.75),
  Maximum = sapply(athens[, -c(1:7)], max) 
)
...
summary_table <- gt(summary_df) %>%
  ...
  tab_header(
    ...
    subtitle = "Table displaying summary statistics for Athens dataset"
  )
print(summary_table)
#
...
# Histograms
hist(athens$realSum, main = "Distribution of Total Price", xlab = "Total Price (realSum)", col = "pink")
hist(athens$cleanliness_rating, main = "Distribution of Cleanliness Rating", xlab = "Cleanliness Rating", col = "lightgreen")
hist(athens$guest_satisfaction_overall, main = "Distribution of Guest Satisfaction", xlab = "Overall Guest Satisfaction", col = "salmon")
hist(athens$dist, main = "Distribution of Distance from City Center", xlab = "Distance (km)", col = "lightyellow")
hist(athens$metro_dist, main = "Distribution of Distance from Metro", xlab = "Distance (km)", col = "lavender")
```
...
```{r}
# Box plots
boxplot(athens$realSum, main = "Box Plot of Total Price", col = "pink")
# Box plot for cleanliness rating
boxplot(athens$cleanliness_rating, main = "Box Plot of Cleanliness Rating", col = "lightgreen")
# Box plot for guest satisfaction overall
boxplot(athens$guest_satisfaction_overall, main = "Box Plot of Guest Satisfaction Overall", col = "salmon")
# Box plot for distance from city center
boxplot(athens$dist, main = "Box Plot of Distance from City Center", col = "lightyellow")
# Box plot for distance from metro station
boxplot(athens$metro_dist, main = "Box Plot of Distance from Metro Station", col = "lavender")
```
...
bar_sf <- st_as_sf(athens, coords = c("lng", "lat"), crs = 4326)
st_geometry(bar_sf)
st_crs(bar_sf)
...
car2 <- hglm(fixed=form_pre, random= ~ 1|X, data=ubar_sf, family = gaussian(), rand.family=CAR(D=W))
car2
```
...
#Spatial Autoregressive Model Fitting
```{r}
...
bar_sf <- st_as_sf(amsterdam, coords = c("lng", "lat"), crs = 4326)
