...
budapest_weekdays <- read.csv("~/GitHub/AirBnb-Spatial-project/data sets/budapest_weekdays.csv")
budapest_weekends <- read.csv("~/GitHub/AirBnb-Spatial-project/data sets/budapest_weekends.csv")
budapest <- rbind(budapest_weekdays, budapest_weekends)
str(budapest)
```
...
summary_df <- data.frame(
  Variable = names(budapest)[-c(1:7)],  
  Minimum = sapply(budapest[, -c(1:7)], min),  
  Q1 = sapply(budapest[, -c(1:7)], quantile, probs = 0.25),  
  Median = sapply(budapest[, -c(1:7)], median),  
  Mean = sapply(budapest[, -c(1:7)], mean),  
  Q3 = sapply(budapest[, -c(1:7)], quantile, probs = 0.75),
  Maximum = sapply(budapest[, -c(1:7)], max) 
)
...
summary_table <- gt(summary_df) %>%
  ...
  tab_header(
    ...
    subtitle = "Table displaying summary statistics for Budapest dataset"
  )
...
# Histograms
hist(budapest$realSum, main = "Distribution of Total Price", xlab = "Total Price (realSum)", col = "pink")
hist(budapest$cleanliness_rating, main = "Distribution of Cleanliness Rating", xlab = "Cleanliness Rating", col = "lightgreen")
hist(budapest$guest_satisfaction_overall, main = "Distribution of Guest Satisfaction", xlab = "Overall Guest Satisfaction", col = "salmon")
hist(budapest$dist, main = "Distribution of Distance from City Center", xlab = "Distance (km)", col = "lightyellow")
hist(budapest$metro_dist, main = "Distribution of Distance from Metro", xlab = "Distance (km)", col = "lavender")
```


```{r}
# Box plots
boxplot(budapest$realSum, main = "Box Plot of Total Price", col = "pink")
# Box plot for cleanliness rating
boxplot(budapest$cleanliness_rating, main = "Box Plot of Cleanliness Rating", col = "lightgreen")
# Box plot for guest satisfaction overall
boxplot(budapest$guest_satisfaction_overall, main = "Box Plot of Guest Satisfaction Overall", col = "salmon")
# Box plot for distance from city center
boxplot(budapest$dist, main = "Box Plot of Distance from City Center", col = "lightyellow")
# Box plot for distance from metro station
boxplot(budapest$metro_dist, main = "Box Plot of Distance from Metro Station", col = "lavender")
```

```{r}
#Creating geomtetry points

bar_sf <- st_as_sf(budapest, coords = c("lng", "lat"), crs = 4326)
st_geometry(bar_sf)
st_crs(bar_sf)
```
...
form_pre <- bar_sf$guest_satisfaction_overall ~ log(bar_sf$realSum) + bar_sf$host_is_superhost + bar_sf$attr_index + bar_sf$person_capacity + bar_sf$dist
