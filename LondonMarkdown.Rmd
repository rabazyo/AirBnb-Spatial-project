...
london <- read.csv("~/GitHub/AirBnb-Spatial-project/data sets/london_weekdays.csv")
str(london)
```
...
summary_df <- data.frame(
  Variable = names(london)[-c(1:7)],  
  Minimum = sapply(london[, -c(1:7)], min),  
  Q1 = sapply(london[, -c(1:7)], quantile, probs = 0.25),  
  Median = sapply(london[, -c(1:7)], median),  
  Mean = sapply(london[, -c(1:7)], mean),  
  Q3 = sapply(london[, -c(1:7)], quantile, probs = 0.75),
  Maximum = sapply(london[, -c(1:7)], max) 
)
...
summary_table <- gt(summary_df) %>%
  ...
  tab_header(
    ...
    subtitle = "Table displaying summary statistics for London dataset"
  )
...
# Histograms
hist(london$realSum, main = "Distribution of Total Price", xlab = "Total Price (realSum)", col = "pink")
hist(london$cleanliness_rating, main = "Distribution of Cleanliness Rating", xlab = "Cleanliness Rating", col = "lightgreen")
hist(london$guest_satisfaction_overall, main = "Distribution of Guest Satisfaction", xlab = "Overall Guest Satisfaction", col = "salmon")
hist(london$dist, main = "Distribution of Distance from City Center", xlab = "Distance (km)", col = "lightyellow")
hist(london$metro_dist, main = "Distribution of Distance from Metro", xlab = "Distance (km)", col = "lavender")
```


```{r}
# Box plots
boxplot(london$realSum, main = "Box Plot of Total Price", col = "pink")
# Box plot for cleanliness rating
boxplot(london$cleanliness_rating, main = "Box Plot of Cleanliness Rating", col = "lightgreen")
# Box plot for guest satisfaction overall
boxplot(london$guest_satisfaction_overall, main = "Box Plot of Guest Satisfaction Overall", col = "salmon")
# Box plot for distance from city center
boxplot(london$dist, main = "Box Plot of Distance from City Center", col = "lightyellow")
# Box plot for distance from metro station
boxplot(london$metro_dist, main = "Box Plot of Distance from Metro Station", col = "lavender")
```

```{r}
#Creating geomtetry points

bar_sf <- st_as_sf(london, coords = c("lng", "lat"), crs = 4326)
st_geometry(bar_sf)
st_crs(bar_sf)
```
